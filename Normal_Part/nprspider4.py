#-*-coding:utf-8-*-
# add more data from the catagories not in the selected before
import requests
...
needed_genres = ['biography-memoir',
 ...
 'travel',
 'young-adults']
genres = {i:genres0[i] for i in genres0 if i not in needed_genres}
with open('genres_urls4.pc','wb') as f:
    pc.dump(genres,f)
...
#        if start>=1500:
#            break
    with open('./pages4/cat_'+g+'_pages4.pc','wb') as f:
        pc.dump(pages,f)
...
#    genres=pc.load(f)
for g in genres:
    with open('./pages4/cat_'+g+'_pages4.pc','rb') as f:
        pages=pc.load(f)
    index_in_origen=0
    for pg in pages:
...
for id in booklist.index:
    ...
    if len(allpages)%100==0:
        ...
        #time.sleep(5)
    #time.sleep(1)
with open('allpages4.pc','wb') as f:
    pc.dump(allpages,f)
...
with open('booklist.pc','rb') as f:
    booklist=pc.load(f)
with open('allpages4.pc','rb') as f:
    allpages=pc.load(f)
